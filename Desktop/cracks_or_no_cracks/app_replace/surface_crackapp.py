...
#    full_picture_file = picture_shower_path + picture_file
    ...
    if predictions[0] == 0:
        answer = "No surface crack found rating GREEN"

    if predictions[0] == 1:
        answer = "Thick crack"
        prediction_f = model_f_layer.predict_classes(images,batch_size=2)
        if prediction_f[0] == 0:
                answer ="Thick diagonal crack this is dangerous maintenance needed immediately rating RED "
        if prediction_f[0] == 1:
                answer ="Thick horizontal crack this is dangerous maintenance needed immediately rating RED"
        if prediction_f[0] == 2:
                answer ="Thick vertical crack this is dangerous maintenance needed immediately rating RED "
    if predictions[0] == 2:
        answer = "Thin crack"
        prediction_f = model_f_layer.predict_classes(images,batch_size=2)
        if prediction_f[0] == 0:
                answer ="Thin diagonal crack this is not too dangerous minor  maintenance needed rating AMBER "
        if prediction_f[0] == 1:
                answer ="Thin horizontal crack this is not too  dangerous minor maintenance needed rating AMBER"
        if prediction_f[0] == 2:
                answer ="Thin vertical crack this is not too  dangerous minor  maintenance needeed rating AMBER"
    ...
    return render_template('fix_rotation.html', answer= answer, full_path=full_path)
...
def query():
    ...
    p.add_to(to_email)
    p.add_cc(cc_email)
    message= "Customer:"+mail+ "your query:"+""+classification+" "+reason+". "+ "This is the query receipt. The incorrectly classified image is attached to the email " 
    #print(message)
